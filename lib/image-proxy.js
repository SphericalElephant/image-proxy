...
module.exports = function () {
    ...
    app.get('/', function (req, res, next) {
        ...
        if (!req.query.f)
            return res.status(400).send('No format string set!');
        ...
        const retrieve = function (remote) {
            ...
            const request = agent.get(options, function (response) {
                ...
                response.on('end', () => {
                    sharpen(data, format, (err, sharpened) => {
                        if (err) return res.status(400).send('Invalid format string ' + err);
                        sharpened.toBuffer((err, buffer, info) => {
                            if (err) return next(err);
                            res.writeHead(200, {
                                'Content-Type': mimeType,
                                'Cache-Control': 'max-age=31536000, public', // 1 year TODO: make configurable?
                            });
                            res.write(buffer, () => {
                                if (nconf.get('cache')) {
                                    cache(nconf.get('cacheFolder'), buffer, format, fileExtension, (err, fileName) => {
        ...
        const getCacheFile = (buffer, format) => {
            const checkSum = md5(buffer);
            return checkSum + '_' + format.split(',').join('_');
        };

        const cache = (cachePath, buffer, format, fileExtension, cb) => {
            const fileName = getCacheFile(buffer, format);
            const cacheFilePath = path.join(cachePath, fileName + '.' + fileExtension);
